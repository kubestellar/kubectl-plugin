...
kubectl multi bindingpolicy list
kubectl multi bp list

# Create a basic binding policy
kubectl multi bindingpolicy create my-policy --clusters cluster1,cluster2

# Create policy with cluster label selectors
kubectl multi bp create web-policy --cluster-labels env=production,tier=web

# Create policy with workload label selectors  
kubectl multi policy create app-policy --workload-labels app=nginx,version=latest

# Create advanced policy with match expressions
kubectl multi binding create advanced-policy \
  --cluster-match-expression "zone In us-east-1,us-west-1" \
  --workload-match-expression "app Exists"

# Add clusters to existing policy
kubectl multi bp add-clusters my-policy cluster3 cluster4

# Remove clusters from policy
kubectl multi bp remove-clusters my-policy cluster1

# Update policy with new labels
kubectl multi bp update-labels my-policy \
  --cluster-labels "env=staging" \
  --workload-labels "tier=backend"

# Delete a binding policy
kubectl multi bp delete my-policy`,
	}
...
kubectl multi bp list --namespace production

# List policies across all namespaces
kubectl multi bp list --all-namespaces`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return operations.ListBindingPolicies(kubeconfig, remoteCtx, namespace)
		},
	}
...
kubectl multi bp create my-policy --clusters cluster1,cluster2

# Create policy for all clusters
kubectl multi bp create global-policy --all-clusters

# Create policy with cluster labels
kubectl multi bp create prod-policy --cluster-labels env=production,zone=us-east

# Create policy with cluster match expressions
kubectl multi bp create advanced-policy \
...
kubectl multi bp delete my-policy --namespace production`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			kubeconfig, remoteCtx, _, namespace, _ := GetGlobalFlags()
			return operations.DeleteBindingPolicy(kubeconfig, remoteCtx, namespace, args[0])
		},
	}
...
kubectl multi bp add-clusters my-policy cluster1 cluster2

# Add cluster in specific namespace
kubectl multi bp add-clusters my-policy cluster3 --namespace production`,
		Args: cobra.MinimumNArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			kubeconfig, remoteCtx, _, namespace, _ := GetGlobalFlags()
			policyName := args[0]
			clusterNames := args[1:]
			return operations.UpdateBindingPolicyWithClusters(kubeconfig, remoteCtx, namespace, policyName, clusterNames, []string{})
		},
	}
...
kubectl multi bp remove-clusters my-policy cluster1 cluster2

# Remove cluster in specific namespace
kubectl multi bp remove-clusters my-policy cluster3 --namespace production`,
		Args: cobra.MinimumNArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			kubeconfig, remoteCtx, _, namespace, _ := GetGlobalFlags()
			policyName := args[0]
			clusterNames := args[1:]
			return operations.UpdateBindingPolicyWithClusters(kubeconfig, remoteCtx, namespace, policyName, []string{}, clusterNames)
		},
	}
...
kubectl multi bp update-labels my-policy --cluster-labels env=staging,zone=us-east

# Update workload labels
kubectl multi bp update-labels my-policy --workload-labels app=nginx,version=v2
  ...
  --workload-labels "app=web-server"`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return operations.UpdateBindingPolicyLabels(
				kubeconfig, remoteCtx, namespace, policyName,
