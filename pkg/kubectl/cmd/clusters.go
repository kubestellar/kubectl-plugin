...
kubectl multi c list

# Register a new cluster  
kubectl multi clusters add my-cluster https://cluster.example.com

# Show detailed cluster information
kubectl multi clusters info my-cluster

# Update cluster labels
kubectl multi clusters label my-cluster env=production zone=us-east

# Remove a cluster from ITS
kubectl multi c remove my-cluster`,
	}
...
kubectl multi clusters list --include-wds
...
kubectl multi clusters add my-cluster

# Register with endpoint and labels
kubectl multi clusters add prod-east \
  --endpoint https://prod-east.example.com \
  --labels env=production,zone=us-east-1

# Short form
kubectl multi c add staging-cluster`,
...
kubectl multi clusters remove my-cluster

# Short forms
kubectl multi c rm old-cluster
kubectl multi c delete staging-cluster`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			kubeconfig, itsContext, _, _, _ := GetGlobalFlags()
...
# Short forms
kubectl multi c info prod-cluster
kubectl multi c describe staging-cluster`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			kubeconfig, itsContext, _, _, _ := GetGlobalFlags()
...
kubectl multi clusters label my-cluster env=production zone=us-east-1

# Update with overwrite
kubectl multi clusters label my-cluster env=staging --overwrite

# Short form
kubectl multi c label prod-cluster tier=web`,
