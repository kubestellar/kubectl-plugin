...
kubestellar helm install webapp ./charts/webapp \
  --set image.tag=v2.0 \
  --set replicas=3

# Install with values file
kubestellar helm install app ./chart --values values.yaml

# Install and create namespace
kubestellar h install monitoring prometheus/kube-prometheus-stack \
...
kubestellar helm upgrade webapp ./charts/webapp \
  --set image.tag=v2.1 \
  --set replicas=5

# Upgrade or install if not exists
kubestellar h upgrade app ./chart --install`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			kubeconfig, remoteCtx, _, namespace, _ := GetGlobalFlags()
...
kubestellar helm list --all-namespaces

# List only deployed releases
kubestellar h list --deployed

# List failed releases
kubestellar h ls --failed`,
		RunE: func(cmd *cobra.Command, args []string) error {
			kubeconfig, remoteCtx, _, namespace, _ := GetGlobalFlags()
...
kubestellar helm uninstall webapp --keep-history

# Short forms
kubestellar h del old-app
kubestellar h un deprecated-service`,
...
kubestellar helm rollback nginx

# Rollback to specific revision
kubestellar helm rollback webapp 3

# Short form
kubestellar h rb app 2`,
...
kubestellar helm status nginx

# Short form
kubestellar h stat webapp`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			kubeconfig, remoteCtx, _, namespace, _ := GetGlobalFlags()
...
kubestellar helm values nginx

# Show all values (computed)
kubestellar helm values webapp --all

# Show values for specific revision
kubestellar h vals app --revision 2`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			kubeconfig, remoteCtx, _, namespace, _ := GetGlobalFlags()
